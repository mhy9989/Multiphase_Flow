...
def measure_throughput(model, input_dummy):
    ...
    if isinstance(input_dummy, tuple):
        ...
        _, _, H, W = input_dummy[0].shape
        bs, repetitions = get_batch_size(H, W)
        _input = torch.rand(bs, 1, H, W).to(input_dummy[0].device)
        input_dummy[0] = _input
        input_dummy = tuple(input_dummy)
    else:
        _, _,  H, W = input_dummy.shape
        bs, repetitions = get_batch_size( H, W)
        input_dummy = torch.rand(bs, 1, H, W).to(input_dummy.device)
    total_time = 0
    with torch.no_grad():
