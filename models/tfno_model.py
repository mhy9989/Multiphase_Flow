...
class TFNO_Model(nn.Module):
   def __init__(self, data_shape, n_modes, n_layers, hid, in_ch, out_ch, 
      ...
      self.operator = nn.Sequential(
               *[TFNO(n_modes=n_modes, hidden_channels=hid,
                  in_channels=hid, out_channels=hid,
                  factorization='tucker',implementation='factorized',rank=0.05) for _ in range(n_layers)],
                )
